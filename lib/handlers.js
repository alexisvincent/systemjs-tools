  ...
  const handlers = {
    ...
    bundle: (options = {}) => (req, res, next) => {
      ...
      if ((typeof bundleTrigger === 'string' && req.originalUrl.endsWith(bundleTrigger)) ||
        ...
        _.bundle(config.entries.join(' + ')).then(({source, sourceMap}) => res.end(
          config.builder.options.sourceMaps === 'inline' || !sourceMap
            ? source
            : source + `\n//# sourceMappingURL=${bundleTrigger}.map`))
      } else if (req.originalUrl.endsWith('.map') &&
        ((typeof bundleTrigger === 'string' && req.originalUrl.indexOf(bundleTrigger) > 0) ||
         (typeof bundleTrigger == 'function' && bundleTrigger(req)))) {
        _.log(`requesting ${req.url} retrieving sourcemap`)
        _.bundle(config.entries.join(' + ')).then(({sourceMap}) => res.end(sourceMap))
      } else {
        next()
      }
    },
