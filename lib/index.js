...
const setupHMR = (tools) => {
    if (tools.hmr) {
        ...
        socket.on('connect', soc => {
            soc.on('identification', console.log.bind(console, 'connected client: '))
        ...
        tools.watcher.on('all', (event, changePath) => {
            changePath = path.relative(tools._baseURL, changePath)
...
const make = (options) => {
    ...
    const tools = { ...optionDefaults, ...options}

    // Construct builder config from defaults and provided options
    tools.builderConfig = {...buildOptionDefaults, ...tools.builderConfig}
    ...
    tools._baseURL = tools.packagePath + path.relative(tools.packagePath, tools._loaderConfig.baseURL.substr(7))
    ...
    tools.server = spdy.createServer(require('spdy-keys'), () => "Nothing to see here... yet")
    tools.socket = io(tools.server)

    tools.server.listen(tools.port)
