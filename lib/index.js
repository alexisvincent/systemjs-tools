...
const init = (configOverrides) => {
  ...
  _.loadCache = () => {
    try {
      ...
      _.warning(`Failed to load ${path.join(config.directories.root, config.cache)}`)
      _.cache = {}
    }
  ...
  _.bundle = (expression, options = {}) => (
    _.then('build', () => {
      ...
      bundlePromise.then(m => console.log(m.source))
  ...
  tools.serve = (configOverride = {}) => {
    let {serve: {server, port, keys, handler, finalHandler, dir}} = merge(config, {serve: configOverride})

    handler = handler(tools)
    finalHandler = finalHandler(tools)

    server = server || spdy.createServer(keys, (req, res) => handler(req, res, () => finalHandler));
  ...
  _.fileChanged = (absolutePath) => {
    ...
    const url = path.join(
      _.builder.loader.getConfig().baseURL,
...
// Bust files that have changed since last time systemjs-tools ran
  ...
  if (config.entries.length > 0) {
    ...
    config.entries.forEach(entry => _.bundle(entry, config.builder.options))
  }
