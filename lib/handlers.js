...
export const makeHandlers = ({_, config}) => {
  ...
  const handlers = {
    ...
    bundle: (options = {}) => (req, res, next) => {
      ...
      const matchRequest = (req, match) =>
        typeof match === 'string' && (req.originalUrl.endsWith(match) || req.originalUrl.endsWith(match + '.map')) ||
        typeof match === 'function' && match(req) ||
        typeof match === 'object' && (match instanceof RegExp) && match.exec(req.originalUrl);
      ...
      if (builder) {
        const builderOptions = merge(config.builder.options, builder.options || {});
        if (req.originalUrl.endsWith('.map')) {
          _.log(`requesting ${req.url} retrieving external sourcemap`)
          _.bundle(builder.expression, builderOptions).then(({sourceMap}) => res.end(sourceMap))
        } else {
          _.log(`requesting ${req.url} triggering bundling`)
          _.bundle(builder.expression, builderOptions).then(({source, sourceMap}) => res.end(
            options.sourceMaps === 'inline' || !sourceMap
              ? source
