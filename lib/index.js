...
const init = (configOverrides = {}) => {
  ...
  _.bundle = (expression, options = {}, updateLastAccessed = true) => {
    ...
    const opts = merge(config.builder.options, options)
    const cacheName = `${expression}#${JSON.stringify(opts)}`
  ...
  _.watchFileSystem = (chokidarOptions = {}) => {
    _.watcher = chokidar.watch(config.directories.root, merge({
      ignored: ["**/jspm_packages", "**/node_modules", "**/icons", path.join(config.directories.root, config.cache)],
      ignoreInitial: true
    }, chokidarOptions))
...
// f: bust files that have changed since last time systemjs-tools ran
  ...
  _.events.fileChanged.subscribe(({absolutePath}) => {
